...
\question Prenez maintenant un nouvel objet circulaire et faites l'opération inverse : mesurez le diamètre de cet objet. Vous pouvez stocker ce résultat dans une cellule plus basse : $C20$, par exemple. Puis calculez son périmètre en utilisant la formule suivante :
\[
Périmètre = \pi \times Diamètre
\]
Vous avez deux options
\begin{compactitem}
  \item \textrr{Méthode recommandée} Soit vous utilisez une très bonne valeur approchée de $\pi$. Google Sheet est capable de vous la fournir : vous pouvez écrire la formule \verb|=PI()| dans la cellule $D20$, et vous utilisez cette valeur de $\pi$
  \item Ou bien dans votre formule, vous utilisez la valeur moyenne que vous avez trouvée et notée en $F1$
\end{compactitem}
Notez le résultat dans la cellule $B20$ de la ligne correspondante. Puis, mesurez le périmètre de cet objet avec votre corde ou votre mètre (notez le dans la cellule $E$20) et comparez le résultat que vous avez calculé avec le résultat que vous avez mesuré. Est-ce que les deux résultats sont proches ? Si oui, bravo, vous avez réussi à utiliser la proportionnalité pour faire une estimation du périmètre d'un cercle à partir de son diamètre.
...
\begin{Indication}{mybluebg}{mybluefr}
  [Bonus] Si vous voulez voir de belles visualisations qui permettent de comprendre le lien entre périmètre et aire d'un cercle, je vous conseille de regarder ces 3 vidéos
  \begin{compactitem}
    \item \href{https://www.youtube.com/watch?v=YokKp3pwVFc}{Area of a circle, formula explained} : Explique comment l'aire du cercle peut être "approximée" grâce à l'aire d'un rectangle. \textrr{Vous pouvez être fiers de vous : c'est grosso modo cette méthode que vous avez mise en place dans la section 4}
    \item \href{https://www.youtube.com/watch?v=-UBLutigNMM}{Circle Area (classic visual proof)} : La même méthode que la vidéo du dessus, mais avec une musique héroïque en fond sonore, tout à fait adaptée à la beauté du concept. 
